...
-- The precedence of different operators
...
-- note you must handle whitespaces --> look at token fn from lecture
...
--     AndTerm ::= AndTerm '/\' AndTerm | Negation
--     NotTerm ::= '!' Negation | Factor
--     Factor ::= '(' Formula ')' | 'T' | 'F' | Ident
...
--     AndTerm ::= Negation '/\' AndTerm | Negation
--     Negation ::= '!' Negation | Factor
--     Factor ::= '(' Formula ')' | 'T' | 'F' | Ident
